...
Legend: ✅ pass · ❌ fail · ⚠️ partial · — not applicable · `- [ ]` untested
...
|---|------|-------|----------|-----|-----|
| 1.1 | App starts without crash | Run `pythonw -m gui.app` from `src/` | Window appears with app title | ✅ | ✅ |
| 1.2 | Default state | On launch | Status = Idle/Waiting; Start enabled; mode = Regular | ✅ | ✅ |
...
| # | Test | Steps | Expected | Win | Lin |
|---|------|-------|----------|-----|-----|
| 2.1 | Click every nav item | Click each of the 11 nav items | Correct panel appears; clicked button highlighted | ✅ | ✅ |
| 2.2 | Home returns to main view | Navigate to any panel, click Home | Output text area (or Short Session form) visible | ✅ | ✅ |
...
| 3.5 | Stop recording | Click Stop while recording | Button changes back to "Start"; status shows "Done" | ✅ | ✅ |
...
| 4.3 | Language change | Change UI language dropdown | All UI strings update immediately | - [ ] | ⚠️ switch worked; coordinate miss landed on German instead of Russian |
...
> **BUG B1 (Ubuntu):** "Auto-Start with Windows" label appears in Settings on Ubuntu — should be "Auto-Start on Login" or hidden on non-Windows platforms.
...
| 7.4 | Add Row dialog — fields editable | In Add Row dialog, click each field and type | Text appears in both fields | ✅ | ✅ |
| 7.5 | Tab key moves Source → Replacement | In Add Row dialog, type in Source, press Tab | Focus moves to Replacement field (not Cancel) | ✅ | ❌ Tab goes to Cancel button (Bug B2) |
| 7.6 | Add Row cancel | Open dialog, click Cancel | Dialog closes; no row added | ✅ | ✅ |
| 7.7 | Add Row confirm | Fill both fields, click Confirm | New row appears in dictionary list | - [ ] | - [ ] |
| 7.8 | Add Row multiple times | Open and confirm dialog 3× | Each submission adds a new row without errors | - [ ] | - [ ] |
| 7.9 | Inline edit Source | Click in a Source cell and type | Text updates immediately | - [ ] | - [ ] |
| 7.10 | Inline edit Replacement | Click in a Replacement cell and type | Text updates immediately | - [ ] | - [ ] |
| 7.11 | Delete Row | Select a row, click Delete | Row removed from list | - [ ] | - [ ] |
| 7.12 | Undo | Add a row, click Undo | Row removed; previous state restored | - [ ] | - [ ] |
| 7.13 | Source help button | Click ? next to Source column | Help popup opens with explanation text | - [ ] | ✅ |
| 7.14 | Replacement help button | Click ? next to Replacement column | Help popup opens with explanation text | - [ ] | - [ ] |
| 7.15 | Export CSV | Click Export CSV, choose save path | CSV file created at chosen location | - [ ] | - [ ] |
| 7.16 | Import CSV | Click Import CSV, select valid CSV | Rows added; import result message shown | - [ ] | - [ ] |
| 7.17 | Row right-click context menu | Right-click a Source or Replacement entry field | Context menu: Select All, Copy, Paste | - [ ] | - [ ] |

> **BUG B2 (Ubuntu):** In the Add Row dialog, pressing Tab from the Source field jumps to the Cancel button instead of the Replacement field. Root cause: `focus_force()` on the dialog window leaves focus on the window frame; Tab then hits the first button in the frame's tab order. Fix: auto-focus the Source entry after `_build()`.
...
| # | Test | Steps | Expected | Win | Lin |
|---|------|-------|----------|-----|-----|
| 12.1 | Panel opens | Click Backup & Restore | Backup path, size estimate, and buttons visible | ✅ | ✅ |
...
| # | Test | Steps | Expected | Win | Lin |
|---|------|-------|----------|-----|-----|
| 14.1 | Form visible in Short mode | Switch to Short Session, go to Home | Two text areas: Transcribed text + Translated text; two save buttons | ✅ | ✅ |
...
| # | Test | Steps | Expected | Win | Lin |
|---|------|-------|----------|-----|-----|
| 18.1 | Switch language | Settings → Language → select new language | All labels, buttons, nav items update immediately | - [ ] | ✅ |
...
| ID | Platform | Panel | Description | Status |
|----|----------|-------|-------------|--------|
| B1 | Ubuntu | Settings | "Auto-Start with Windows" label shows on Linux — should read "Auto-Start on Login" or be hidden | Open |
| B2 | Ubuntu | Dictionary | Tab from Add Row Source field goes to Cancel button instead of Replacement field | Open |
...
|------|--------|----------|---------|-------|
| 2026-06-14 | Automated (Claude) | Windows 11 | 0.5.070 | SendInput automation via PowerShell; typing in dialogs not reliable via keybd_event |
| 2026-06-14 | Automated (Claude) | Ubuntu 26.04 | 0.5.070 | xdotool automation; Wayland screenshot via vmrun captureScreen |
